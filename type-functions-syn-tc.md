...
Normalisation of an equality `s = t` of arbitrary type terms `s` and `t` leads to a (possibly empty) set of normal equations, or to a type error.  We proceed as follows:
 ...
 3. If `s'` and `t'` are distinct rigid variables, we fail.
 4. If `s'` or `t'` is a wobbly type variables, instantiate it with the other type (after occurs check).
 5. If `s'` = `C1 ...` and `t' = C2 ...`, where `C1` and `C2` are different data type constructors, we reject the program.
 6. If `s'` = `C s1 .. sn` and `t'` = `C t1 .. tn`, then yield the union of the equations obtained by normalising all `ti = si`.
