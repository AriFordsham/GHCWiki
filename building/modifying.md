...
- **`make TRACE=1`**
  ...
  prints messages about certain macros that are called and their arguments.  This is basically a system of `$(warning)` calls enabled when the value of `$(TRACE)` is non-empty.  To see how it works, look at the file [rules/trace.mk](https://gitlab.haskell.org/ghc/ghc/tree/master/rules/trace.mk), and feel free to add trace calls to more places in the build system.
...
Like any Cabal package, the `ghc` package has a `ghc.cabal` file, except that in this case the file is preprocessed by `configure` from the original: [compiler/ghc.cabal.in](https://gitlab.haskell.org/ghc/ghc/tree/master/compiler/ghc.cabal.in).  Be careful not to modify the preprocessed version, `ghc.cabal`, as it will be overwritten next time you run `configure`.


To add a module to the `ghc` package:

- Add your module to the `exposed-modules` section of [compiler/ghc.cabal.in](https://gitlab.haskell.org/ghc/ghc/tree/master/compiler/ghc.cabal.in)
...
- Remove the module from the list of modules in [compiler/ghc.cabal.in](https://gitlab.haskell.org/ghc/ghc/tree/master/compiler/ghc.cabal.in).
...
- To arrange that the package is known to `./boot`, add an entry to the [packages](https://gitlab.haskell.org/ghc/ghc/tree/master/packages) file.
- run `perl boot` to generate the `ghc.mk` and `GNUmakefile` files in your package's build.
- Add an entry to the `PACKAGES` variable in [ghc.mk](https://gitlab.haskell.org/ghc/ghc/tree/master/ghc.mk).  The list in `PACKAGES` is kept in dependency order: each package must appear after the packages it depends on.
...
This is a call to the `build-prog` macro, whose definition can be found in [rules/build-prog.mk](https://gitlab.haskell.org/ghc/ghc/tree/master/rules/build-prog.mk).  This expands to the code for actually building the program, and it takes three arguments: the directory, the distdir, and the GHC stage to use for building (in this case we're using stage 1).  The `build-prog` macro expects to find certain variables defined, namely *directory*_*distdir*`_PROG`, and *directory*_*distdir*`_MODULES`.
