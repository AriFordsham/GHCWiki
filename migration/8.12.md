...
This relies on deep skolemisation, which no longer exists under simplified subsumption. As a result, GHC 8.12 will reject `h`:
...
Again, this relies on deep skolemisation. As a result, GHC 8.12 will reject `h'`:
...
This requires deeply instantiating the type of `f`. Simplified subsumption gets rid of deep instantiation, however, so `g` will not typecheck on GHC 8.12:
...
This is because GHC's subtyping rules supported contravariance in function arrows, which is something that simplified subsumption removes. As a result, `h` will not typecheck on GHC 8.12:
...
GHC's constraint solver now solves constraints in each top-level group sooner. This has practical consequences for Template Haskell, as TH splices necessarily separate top-level groups. For example, the following program would compile in previous versions of GHC, but not in GHC 8.12:
...
Previous versions of GHC did not specify the order or specificity of type variables in the types of top-level field selectors. GHC 8.12 does specify this, which can impact users of `TypeApplications`. For example, given the following definitions:
...
GHC has a rule that GADT constructors cannot contain nested `forall`s or contexts in GADT constructors. This was always true in previous released, but GHC 8.12 enforces this rule more strictly. In particular, nested `forall`s or contexts that occur underneath parentheses are now rejected. This means the following examples will no longer compile on GHC 8.12:
...
Much like GHC does not permit nested `forall`s or contexts in GADT constructors, it also does not permit them in the types at the tops of instance declarations. GHC also enforces this more strictly in 8.12, so the following examples will no longer compile:
...
A similar no-nested-`forall`s rule applies to types involved in `deriving` clauses and `via` types (for instances derived with `DerivingVia`). The following examples will also be rejected with GHC 8.12:
...
The `unsafeCoerce#` function has been moved from `GHC.Prim` to `Unsafe.Coerce`. As a result, attempting to import `unsafeCoerce#` from `GHC.Prim` (or `GHC.Base`, which previously re-exported `unsafeCoerce#`) will result in an error with GHC 8.12. A backwards-compatible way to fix the error is to import `unsafeCoerce#` from `GHC.Exts` instead.
