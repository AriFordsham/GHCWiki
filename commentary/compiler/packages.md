...
[compiler/main/Packages.lhs](https://gitlab.haskell.org/ghc/ghc/tree/master/ghc/compiler/main/Packages.lhs), and stashed in the `DynFlags`.
...
Source code: [compiler/basicTypes/Module.lhs](https://gitlab.haskell.org/ghc/ghc/tree/master/ghc/compiler/basicTypes/Module.lhs).
...
To find out what the current package is, grab the field `thisPackage` from `DynFlags` (see [compiler/main/DynFlags.hs](https://gitlab.haskell.org/ghc/ghc/tree/master/ghc/compiler/main/DynFlags.hs)).

## Special packages


Certain packages are special, in the sense that GHC knows about their existence and something about their contents.  Any [Name](commentary/compiler/name-type) that is [wired-in](commentary/compiler/wired-in) (see [compiler/prelude/PrelNames.lhs](https://gitlab.haskell.org/ghc/ghc/tree/master/ghc/compiler/prelude/PrelNames.lhs)) must by definition contain a `Module`, and that module must therefore contain a `PackageId`.  But the `PackageId` includes the version, so does this mean we have to somehow find out the version of the `base` package (for example) and bake it into the GHC binary?
...
The definition of which packages are special and the stripping of versions from the package database happens in `findWiredInPackages` in [compiler/main/Packages.lhs](https://gitlab.haskell.org/ghc/ghc/tree/master/ghc/compiler/main/Packages.lhs): we literally just rewrite the `sourcePackageId` associated with the package.
...
Source code: see the `Outputable` instance for `Module` in [compiler/basicTypes/Module.lhs](https://gitlab.haskell.org/ghc/ghc/tree/master/ghc/compiler/basicTypes/Module.lhs).
