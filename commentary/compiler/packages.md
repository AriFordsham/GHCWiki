...
[compiler/main/Packages.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/main/Packages.hs), and stashed in the `DynFlags`.
...
Source code: [compiler/basicTypes/Module.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/basicTypes/Module.hs).
...
The definition of which packages are special and the stripping of versions from the package database happens in `findWiredInPackages` in [compiler/main/Packages.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/main/Packages.hs): we literally just rewrite the `sourcePackageId` associated with the package.
...
Source code: see the `Outputable` instance for `Module` in [compiler/basicTypes/Module.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/basicTypes/Module.hs).
