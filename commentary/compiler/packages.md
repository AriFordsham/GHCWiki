...
Source files: [compiler/GHC/Unit/Info.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Unit/Info.hs), [compiler/GHC/Unit/State.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Unit/State.hs)
...
[compiler/GHC/Unit/State.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Unit/State.hs), and stashed in the `DynFlags`.
...
Source code: [compiler/GHC/Unit/Module.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Unit/Module.hs).
...
To find out what the current package is, grab the field `thisPackage` from `DynFlags` (see [compiler/GHC/Driver/Session.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Driver/Session.hs)).

## Special packages


Certain packages are special, in the sense that GHC knows about their existence and something about their contents.  Any [Name](commentary/compiler/name-type) that is [wired-in](commentary/compiler/wired-in) (see [compiler/GHC/Builtin/Names.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Builtin/Names.hs)) must by definition contain a `Module`, and that module must therefore contain a `PackageId`.  But the `PackageId` includes the version, so does this mean we have to somehow find out the version of the `base` package (for example) and bake it into the GHC binary?
...
The definition of which packages are special and the stripping of versions from the package database happens in `findWiredInPackages` in [compiler/GHC/Unit/State.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Unit/State.hs): we literally just rewrite the `sourcePackageId` associated with the package.
...
Source code: see the `Outputable` instance for `Module` in [compiler/GHC/Unit/Module.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Unit/Module.hs).
