...
- Log performance results (using [git notes](https://git-scm.com/docs/git-notes)) for all test runs.
...
- When running performance tests:
  ...
  - All test results (tagged with the current platform) are appended to the [git note](https://git-scm.com/docs/git-notes) for the current commit.
- The CI server will automatically record performance results for each merged commit, pushing the performance results to the git.haskell.com repo.
