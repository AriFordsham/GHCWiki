...
For a type constructor `T` and its data constructors `C`, we have three alternatives:
 ...
 2. The converted variant `T_CC` coincides with `T` (e.g., because `T` neither directly nor indirectly involves arrows).
 3. The converted variant `T_CC` differs from `T`.
