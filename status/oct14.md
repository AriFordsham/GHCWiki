...
- **Propositional Equality for `GHC.Generics` Metadata** - Gabor Greif is working on a conservative approach to retrofit the metadata types generated by `-XDeriveGenerics` with a runtime-observable type equality. The feature is discussed under [GenericsPropositionalEquality](generics-propositional-equality).
