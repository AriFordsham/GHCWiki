...
- Currently, all fingerprints match between the old, `base`, `TypeRep`, the new `TypeRep` and the compatability `TypeRep710`.
  Is this the correct route, or should we force them to be different, or perhaps different in a predictable way (xor with "new_", "old_" and "compat" or some-such)?
