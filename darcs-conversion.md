...
Google has a tool called [ repo](https://sites.google.com/a/android.com/opensource/download/using-repo), which is basically the same as our `darcs-all` script but is much much larger (it probably does a bit more, to be fair).  It is written in Python and the list of git repositories is kept in an XML file.
